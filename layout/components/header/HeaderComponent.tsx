...
const HeaderComponent: React.FC<Props> = (props) => {
  ...
  const items: MenuProps["items"] = [
    {
      label: <span>{session?.user.name ? session?.user.name : parseJwt(session?.user.access_token).Email}</span>,
      key: "0",
    },
        ...
        >
          <Space>
            <Avatar style={{ backgroundColor: "#fde3cf", color: "#f56a00" }}>
              {session?.user.userName ? session?.user.userName?.charAt(0) : parseJwt(session?.user.access_token).Email.charAt(0)}
            </Avatar>
          </Space>
        </Dropdown>
      </div>
