...
const StatisticPage: React.FC = () => {
  ...
  const handleChange = (value: number) => {
    ...
    switch (value) {
      case 1:
        ...
        endDate1 = dayjs(`${selectedYear}/01/31`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate2 = isLeapYear(selectedYear)
          ? dayjs(`${selectedYear}/02/29`).endOf("day").format(dateAdvFormat)
          : dayjs(`${selectedYear}/02/28`).endOf("day").format(dateAdvFormat);
        endDate3 = dayjs(`${selectedYear}/03/31`)
          .endOf("day")
          .format(dateAdvFormat);
      ...
      case 2:
        ...
        endDate1 = dayjs(`${selectedYear}/04/30`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate2 = dayjs(`${selectedYear}/05/31`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate3 = dayjs(`${selectedYear}/06/30`)
          .endOf("day")
          .format(dateAdvFormat);
        setStart1(startDate1);
        setStart2(startDate2);
      ...
      case 3:
        ...
        endDate1 = dayjs(`${selectedYear}/07/31`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate2 = dayjs(`${selectedYear}/08/31`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate3 = dayjs(`${selectedYear}/09/30`)
          .endOf("day")
          .format(dateAdvFormat);
        setStart1(startDate1);
        setStart2(startDate2);
      ...
      case 4:
        ...
        endDate1 = dayjs(`${selectedYear}/10/31`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate2 = dayjs(`${selectedYear}/11/30`)
          .endOf("day")
          .format(dateAdvFormat);
        endDate3 = dayjs(`${selectedYear}/12/31`)
          .endOf("day")
          .format(dateAdvFormat);
        setStart1(startDate1);
        setStart2(startDate2);
  ...
  return (
    <AntdLayoutNoSSR
      content={
        <>
          {areInArray(session?.user.roles!, ROLE_SALES, ROLE_MANAGER) && (
            <>
                ...
                {selectedYear && selectedYear > 0 && (
                  <>
                    ...
                    >
                      <Option
                        value={1}
                      >{`First quarter (January - March)`}</Option>
                      <Option
                        value={2}
                      >{`Second quarter (April - June)`}</Option>
                      <Option
                        value={3}
                      >{`Third quarter (July - September)`}</Option>
                      <Option
                        value={4}
                      >{`Fourth quarter (October - December)`}</Option>
                    </Select>
                  </>
                )}
              </div>
              <div className="flex justify-center">
                <span className="m-2" style={{ textAlign: "center" }}>
                  <BarChartComponent barData={barDataMonth1!} />
                  <h3>{month1}</h3>
                </span>
                <span className="m-2" style={{ textAlign: "center" }}>
                  <BarChartComponent barData={barDataMonth2!} />
                  <h3>{month2}</h3>
                </span>
                <span className="m-2" style={{ textAlign: "center" }}>
                  <BarChartComponent barData={barDataMonth3!} />
                  <h3>{month3}</h3>
                </span>
              </div>
            </>
