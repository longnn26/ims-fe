...
const ProductInfoPage: React.FC<Props> = (props) => {
  ...
  const [destLocationOptions, setDestLocationOptions] = useState<OptionType[]>(
    []
  );
  ...
  const onSave = async () => {
    ...
    if (stockPickingId === "new") {
      const data =
        formStockPicking.getFieldsValue() as StockPickingInternalTransfer;
      data.scheduledDate = scheduledDate;
      data.dateDeadline = dateDeadline;
      await stockPickingServices
        .createStockPickingInternalTransfer(accessToken, data)
        .then((res) => {
          router
            .push(`/overview/${warehouseId}/internal/${res?.id}`)
            .then(() => {
              router.reload();
            });
        })
      ...
      await stockLPickingServices
        .updateStockPickingInternalTransfer(accessToken, {
          id: stockPickingId,
          note: formStockPicking.getFieldsValue().note,
          locationId: formStockPicking.getFieldsValue().locationId,
          locationDestId: formStockPicking.getFieldsValue().locationDestId,
          dateDeadline: dateDeadline,
          scheduledDate: scheduledDate,
        } as StockPickingInternalTransferUpdate)
        .then(() => {
  ...
  const fetchDestLocations = async () => {
    await stockLocationServices
      ...
      .then((res) => {
        ...
        setDestLocationOptions(options);
      })
      .catch((error) => {
        message.error(error?.response?.data);
      });
                  ...
                  <Col span={12}>
                    ...
                    >
                      <Select
                        ...
                        variant="filled"
                      >
                        {destLocationOptions.map((option) => (
                          <Option key={option.value} value={option.value}>
                            {option.label}
                          </Option>
                        ))}
