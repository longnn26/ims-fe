...
const ProductInfoPage: React.FC<Props> = (props) => {
  ...
  const onSave = async () => {
    ...
    if (stockPickingId === "new") {
      const data =
        formStockPicking.getFieldsValue() as StockPickingDeliveryOrder;
      data.scheduledDate = scheduledDate;
      data.dateDeadline = dateDeadline;
      await stockPickingServices
        .createStockPickingDeliveryOrder(accessToken, data)
        .then((res) => {
          router
            .push(`/overview/${warehouseId}/outgoing/${res?.id}`)
            .then(() => {
              router.reload();
            });
        })
