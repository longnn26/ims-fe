...
const StockQuantLocationTable: React.FC<Props> = (props) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const { accessToken, locationId } = props;
  const {
    data: stockQuantData,
    loading,
    pageIndex,
  ...
  const fetchStocQuantLocationData = useCallback(() => {
    dispatch(
      getStockQuants({
        token: accessToken,
...
export default StockQuantLocationTable;
